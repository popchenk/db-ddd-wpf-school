...
namespace SQLiteWPF
{
    class SetupSQLite
    {
        ...
        public void CreateSQLiteDatabase()
        {
            ...
            SQLiteFunction TestingFunction = new TestingFunction();
            var attributes = TestingFunction.GetType().GetCustomAttributes(typeof(SQLiteFunctionAttribute), true).Cast<SQLiteFunctionAttribute>().ToArray();
            if (attributes.Length == 0)
            {
                throw new InvalidOperationException("SQLiteFunction doesn't have SQLiteFunctionAttribute");
            }
            sqliteconn.BindFunction(attributes[0], TestingFunction);
